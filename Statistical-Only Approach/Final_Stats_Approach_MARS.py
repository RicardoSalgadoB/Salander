...
def process_seismic_file_and_generate_csv(csv_file):
   ...
   if len(peaks) > 0:
       ...
       event_start_time = rel_time[peaks[np.argmax(velocity[peaks])]]  # Hora de inicio del sismo
       ...
       event_end_time = rel_time[peaks[-1]]  # Hora del fin del sismo (basado en el último pico relevante)
   else:
